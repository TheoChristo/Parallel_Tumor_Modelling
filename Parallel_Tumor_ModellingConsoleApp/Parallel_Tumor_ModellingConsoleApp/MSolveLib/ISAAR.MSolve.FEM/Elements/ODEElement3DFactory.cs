...
{
    public class ODEElement3DFactory
    {
        ...
        private readonly ODEMaterial commonMaterial;

        static ODEElement3DFactory()
        {
            ...
            ODEElement3DFactory.interpolations = interpolations;
            ODEElement3DFactory.integrationsForStiffness = integrationsForStiffness;
            ODEElement3DFactory.integrationsForMass = integrationsForMass;
            ODEElement3DFactory.extrapolations = extrapolations;
        }

        public ODEElement3DFactory(ODEMaterial commonMaterial)
        {
            this.commonMaterial = commonMaterial;
        }

        public ODEElement3D CreateElement(CellType cellType, IReadOnlyList<Node> nodes)
        {
            //TODO: check if nodes - interpolation and Gauss points - materials match
#if DEBUG
            interpolations[cellType].CheckElementNodes(nodes);
#endif
            return new ODEElement3D(nodes, interpolations[cellType],
